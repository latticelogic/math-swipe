...
- **Airwallex: card-method activation + one real purchase** — everything else went LIVE 2026-07-22 (KYB approved 2nd attempt; secrets set; webhook registered; functions deployed with public invokers behind a project-scoped DRS exception; money-free QA loop passed end-to-end — see the status block in `docs/airwallex.md`). Remaining: Airwallex finishes activating Visa/MC/Amex/UnionPay/Apple Pay/Google Pay (they email per method), then one real $3.14 purchase through the UI + Console refund as the final gate. Note the 4%/30-day rolling reserve on early settlements.
