        ...
        <div className="flex-1 flex flex-col items-center overflow-y-auto px-6 pt-[calc(env(safe-area-inset-top,16px)+28px)] pb-20 relative">
            ...
            {isAnonymous && (() => {
                ...
                const line = hasProgress
                    ...
                    : t('me.signinBody');
                // Apple sign-in: the env flag (web, once the provider is enabled) OR the
                // native bridge (iOS shell — ASAuthorization, always available there).
                const appleEnabled = import.meta.env.VITE_ENABLE_APPLE === '1'
                    || typeof (window as { AppleAuth?: unknown }).AppleAuth !== 'undefined';
                        ...
                        {showEmailInput ? (
                            ...
                            <>
                                ...
                                <div className="flex flex-col gap-2">
                                    {/* Google sign-in uses web OAuth, which Google BLOCKS inside an
                                        embedded WebView (the native shell) — it dead-ends on a white
                                        screen. Hide it there until the native Google Sign-In bridge
                                        (window.AndroidAuth, docs/native-android-plan.md) ships; then
                                        this shows again and routes through the bridge. The plain web
                                        app + TWA are unaffected. */}
                                    {/* Google OAuth dead-ends in embedded WebViews: hidden in the iOS shell
                                        (native Apple sign-in covers it there) and on native Android
                                        without the AndroidAuth bridge. */}
                                    {!isNativeIOS() && !(isNativeAndroid() && typeof (window as { AndroidAuth?: unknown }).AndroidAuth === 'undefined') && (
